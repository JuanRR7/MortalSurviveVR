...
public class LogicaArma : MonoBehaviour {
    ...
    public GeneradorDeEnemigos generadorDeEnemigos;//acceder a otro script
    ...
    public int numeroOleadaAnterior;//creadas para comparar oleada
    ...
    void Start() {
        ...
        // Obtener el número de oleada desde GeneradorDeEnemigos
        numeroOleada = generadorDeEnemigos.numeroOleada;
    ...
    void Update() {
        ...
        //Comparar la Oleada para saber si comenzo una nueva
        if (generadorDeEnemigos.numeroOleada != numeroOleadaAnterior)
        {
            numeroOleadaAnterior = generadorDeEnemigos.numeroOleada;

            // Actualizar maximoDeBalas según el número de oleada
             if ( numeroOleadaAnterior <= 15)
            {
                maximoDeBalas = 100;  
            }
            else if (numeroOleadaAnterior == 16)
            {
                maximoDeBalas = 135;  
            }else if (numeroOleadaAnterior >= 17)
            {
                maximoDeBalas = 155;  
            }

            // restablecer las balas restantes y en cartucho si lo deseas
            balasEnCartucho = tamañoDeCartucho;
            balasRestantes = maximoDeBalas;
        }
    }
    ...
    private void OnTriggerEnter(Collider other){
     if (other.CompareTag("MasBalas")) {
            audioSource.PlayOneShot(powerUp);
             Debug.Log("HAN COLISIONADO");
             restablecer = true;
            Destroy(other.gameObject);
            balasRestantes= ((balasRestantes+100)-balasRestantes);
        }
    }
